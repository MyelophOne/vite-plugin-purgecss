...
describe('purgecss font-face', () => {
	...
	beforeAll(async () => {
		const results = await viteBuildCss(root, [pluginPurgeCss({ fontFace: true })])
		output = results['index.css']
	})

	it("keep @font-face 'Cerebri Bold'", () => {
		expect(output.includes(`src: url("../fonts/CerebriSans-Bold.eot?")`)).toBe(true)
	})

	it("keep @font-face 'Cerebri Sans'", () => {
		expect(output.includes(`src: url("../fonts/CerebriSans-Regular.eot?")`)).toBe(true)
	})
